	...
	unsafe {
		...
		for i in 0..RX_BUF_NUM {
			...
			let buffer = DeviceAlloc
				.phys_addr_from((rxbuffer + u64::from(i * RX_BUF_LEN)).as_mut_ptr::<()>());
		...
		for i in 0..TX_BUF_NUM {
			...
			let buffer = DeviceAlloc
				.phys_addr_from((txbuffer + u64::from(i * TX_BUF_LEN)).as_mut_ptr::<()>());
